...
fn qjs_engine_error(error: agenterm_qjswasm::QjswasmError) -> ScriptEngineError {
    ...
    let category = match &error {
        ...
        | E::NoSuchExport(_)
        | E::Signature(_)
        | E::UnsupportedValue(_) => ScriptFailureCategory::Configuration,
    };
    ScriptEngineError {
        code: None,
...
mod tests {
    ...
    #[test]
    #[cfg(feature = "script-qjswasm")]
    fn qjs_budget_maps_the_public_collection_ceiling() {
        let invocation_budget = ScriptBudgets {
            collection_items: 7,
            ..ScriptBudgets::default()
        };
        ...
        assert_eq!(qjs_budget(&options).max_collection_items, 7);
    }
    ...
    fn qjs_collection_ceiling_governs_source_and_reusable_artifacts() {
        ...
        assert!(source_error.to_string().contains("collection_items"));
