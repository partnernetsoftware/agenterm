...
pub(super) enum SecurityType {
    ...
    Md5Hash = 21,
    ColinDeanXvp = 22,
    /// AGENTERM PATCH: Apple Remote Management, what macOS Screen Sharing
    /// offers when authenticating against a real account.
...
impl TryFrom<u8> for SecurityType {
    ...
    fn try_from(num: u8) -> Result<Self, Self::Error> {
        match num {
            // AGENTERM PATCH: an explicit map replaces a transmute that was
            // unsound for any value outside the listed set.
...
impl From<u32> for AuthResult {
    fn from(num: u32) -> Self {
        // AGENTERM PATCH: upstream transmutes here, which is undefined
        // behaviour for any value outside {0, 1} and aborts the process with a
        // non-unwinding panic no caller can catch. Treat anything unexpected
        // as a failure, which is the safe reading of a malformed result.
        match num {
            0 => AuthResult::Ok,
            _ => AuthResult::Failed,
        }
