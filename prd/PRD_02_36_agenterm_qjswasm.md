...
│   ├── 其它方法（toUpperCase · toFixed · filter · join）     [ ] 字符串读报名（`MissingStringMethod`）；数组读为 undefined，调用它报名（`NotAFunction`）
