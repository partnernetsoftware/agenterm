...
上游 rev **`4917841`**，即 `Cargo.lock` 里的 pin）；
...
buffer、`s + "x"` 的二次拷贝；(3) ~~`undefined.x` 可捕获~~ 已做（`4917841`）；
