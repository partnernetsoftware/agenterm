...
flowchart LR
  ...
  QPTY["ACU headless PTY journey<br/>snapshot · send · raw wait · events · exit · cleanup"]
...
- [x] The platform-neutral `cu-pty-smoke` task drives the public ACU facade
  ...
  epoch-bound structured snapshot and non-empty event/screen continuation,
  ...
  The next local macOS enlargement added `pty-snapshot` before input and
  `pty-events` after the exact output match. It asserts the same durable job,
  stable tab, server epoch and event cursor, plus a non-empty terminal event and
  updated structured screen. This local result does not inherit the earlier
  six-cell claim; the enlarged bytes still require their own six-cell rerun.
