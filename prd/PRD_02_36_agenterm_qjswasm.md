...
> **边界（政委 2026-08-29）**：`.rh` 是独立仓 `partnernetsoftware/rh` 的引擎前端，
> **已暂停开发**，不归本产品线。本产品线 = **`.qjs`（qjswasm）与 `.wasm`（tinyvm）**。
> 本仓 `scripts/rh/` 下的 71 个 `.rh` 脚本与 `crates/agenterm-rh` 不是本清单的对象。
>
> **这句话同时更正了一条方法论错误**：2026-08-29 的两次需求普查都拿那 82 个 `.rh`
> 脚本当语料，并照着排了八个语言里程碑。**那是另一个产品的语料。** 特性本身没做错
> （每个都有实测与门控，且都是通用 JS 能力），但「按它排序」这条依据**不成立**——
> 见上游 PRD「需求普查」两节的更正。今后 `.qjs` 的需求要从 **`.qjs` 自己的用途**里来，
> 而它现在还没有用途（A1）。
...
| # | 事 | 为什么（实测） | 做完算什么 |
|---|----|---------------|-----------|
| A1 | **给 `.qjs` 一个真实的生产用途** | 全仓真实 `.qjs` 只有 `scripts/qjs/lib/fleet.qjs` **一个库、零个任务脚本** | 至少一个 `.qjs` 任务脚本被产品真的跑（不是测试里跑） |
| ~~A2~~ | ~~决定 `.qjs` 与 `.rh` 的关系~~ | **政委 2026-08-29 已答**：rh 是**独立仓** `partnernetsoftware/rh` 的引擎前端，**已暂停开发**；本产品线只管 `.qjs` 与 `.wasm`。**不接管、不迁移、不并存对齐** | 已闭合。A1 的依据因此不是「迁移 rh」，而是**qjswasm 自己的用途** |
