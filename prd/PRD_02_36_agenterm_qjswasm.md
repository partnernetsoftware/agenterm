...
**147 passed / 0 ignored**，2026-08-28，上游 rev **`9e02e37`**）；**`.qjs` 已经够得着
...
| **子集更窄，这是会坏的东西** | 默认参数 / `Math` / 带标签的模板——rquickjs 跑得动、qjswasm 拒绝（**捕获闭包 `68afb35`、模板 `653cebe`、箭头 `9e02e37` 已先后离开这一行**） |
...
`ab29522`（整个 DecimalLiteral）→ `653cebe`（模板字面量）→ **`9e02e37`**（箭头函数，
当前 pin，2026-08-28）。
...
（`653cebe`）、箭头函数（`9e02e37`）都到了。写 fleet 绑定和自动化脚本够用且顺手；
缺的是**方法**，不是语法。
