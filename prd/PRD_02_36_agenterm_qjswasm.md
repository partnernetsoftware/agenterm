...
│   ├── 宿主操作 / 等待节拍预算（A1.12）                            [x] 计数+上限+账单到 CLI（成功与失败都有）；两条旅程：等待仅 8%，步数是真算；剖面：parse 48% / stringify 28%；parse 砍 38% 后旅程 −16%
