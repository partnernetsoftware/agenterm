...
上游 rev **`be1447c`**，即 `Cargo.lock` 里的 pin）；
...
│   ├── 产品脚本手写过的工具，按需求数落地（tinyvm A12）        [x] 2026-08-31 第一批 `c9df46f`：`Array.isArray` · 数组 `indexOf`/`includes` · `concat` · `join`——`rh_compat.qjs` 的 `is_array`/`array_has`、`cu-macos-smoke.qjs` 的 `invoke_arguments`/`path_text` 可退役（脚本本身未改）；第二批 `eb95e42`：`sort([cmp])`——`prune_target_incremental.qjs:317` 与 `check.qjs:683` 的手写归并可退役（引擎的 3.3× 快，1 000 个串 4.2M 步 vs 13.8M）；第三批 `fb1a1a6`：`charCodeAt` / `charAt` / `substring` / 只读 `s[i]`——`rh_compat.qjs:446` 的 `ASCII_PRINTABLE` 表与 `ascii_bytes`、`qualification.qjs:18` 的字符走法、`build-all.qjs:40` 的 `tail_of` 可退役；第四批 `1f86bbc`：`JSON.stringify(v, null, space)`——`rh_compat.qjs:96` 的 `stringify_pretty` 可以真的 pretty；`replacer` 仍具名拒绝。**四批全部落地**；**脚本侧退役 2026-08-31 落地**（`plan/design-host-op-budget.md` §8）：`rh_compat` 的 `is_array`/`is_map`/`array_has`/`list_has_value`/`sort_strings`/`ascii_bytes`（`ASCII_PRINTABLE` 表删除）/`stringify_pretty`（真 pretty）改走引擎，两份手写排序、`invoke_arguments`/`path_text`、`tail_of` 退役，导出名与调用点不变；`only_chars` **留下**——量过 `replaceAll` 删字比逐位 `charCodeAt` 便宜（64 hex 52k 对 93k–113k 步，每次字符访问一次 prefab 调用）；三条旅程 wake −5.0%（轮询里的 `array_has`）、server −0.1%、cu −0.13%，STEP/EVIDENCE 不变
│   ├── 数学 / 位运算 / 格式化工具，按需求数落地（tinyvm A13）   [~] 2026-08-31 第一批 `2e26e3d`：位运算 `& | ^ ~ << >> >>>` + 六个复合赋值——`rh_compat.qjs` 的 `xor16`（16 轮 `% 2` 循环）与 `fnv1a64_hex` 的 16 位 limb 乘可退役（54× 快）；下游调用数：`int_div` 68 行/8 文件、`int_of` 46/7、`only_chars` 14/6、`rfc3339_from_ms` 10/3、`parse_int` 6/4、`fnv1a64_hex` 2/2（`rfc3339_from_ms` 要 `Date`，B 区不做）；第二批 `295a95f`：`Math` 十个函数 + `PI`/`E`——`int_div`/`floor_div` 可退役（`Math.trunc(a/b)` 3.0× 快，缺员编译期报名、分数指数具名拒绝）；第三批 `ea820ef`：`parseInt` + `Number.isInteger`/`isNaN`——`parse_int` 可退役（16× 快）
...
**同一条命令、同一个二进制的账单，按 pin 记**（`--profile tool --max-operations 1000000000`，`steps / host_ops / host_bytes / waited_ms / heap_pages`；步数随环境答复长度有 0.01%–0.1% 噪声）：`d027845`：server-smoke **22 517 385** / 527 / 889 561 / 281 / 32，wake-smoke **37 221 773** / 509 / 1 144 943 / 246 / 43。`c9df46f`（isArray · 数组 indexOf/includes · concat · join）：server-smoke 22 528 222 / 527 / 890 920 / 279 / 32，wake-smoke 37 221 777 / 509 / 1 144 943 / 261 / 43——**两条旅程都不点名这些方法，账单在噪声内不动**（+0.05% / +4 步），这就是「不点名不付钱」在产品脚本上的证据。`eb95e42`（sort）：server-smoke 22 475 710 / 527 / 885 699 / 279 / 32，wake-smoke 37 222 633 / 509 / 1 143 904 / 244 / 43——不动（−0.2% / +0.002%，前者是环境答复短了 3.9 KB）。`fb1a1a6`（字符访问）：server-smoke **22 116 744** / 527 / 890 201 / 279 / 32，wake-smoke **36 151 626** / 509 / 1 144 943 / 260 / 43——**降了 1.6% / 2.9%**，不是噪声：两条旅程都不点名新方法，但都点名 `slice`（server-smoke 1 处、wake-smoke 2 处、`test_harness.qjs` 2 处），而 `slice` 的码元核心这一批学会了八个 ASCII 字节一步。`1f86bbc`（`space`）：server-smoke 22 147 642 / 527 / 890 201 / 276 / 32，wake-smoke 36 178 917 / 509 / 1 144 943 / 242 / 43——+0.14% / +0.08%，在噪声带内（紧凑路每个属性多读一次 gap 字，是这一批唯一落在旅程上的东西）。**四批合计**：server-smoke 22 517 385 → 22 147 642（−1.6%），wake-smoke 37 221 773 → 36 178 917（−2.8%）——两条旅程没点名任何新方法，账单没涨，降的那截是 `slice` 共享的走法。`2e26e3d`（位运算）：**脚本在两个 pin 之间被另一会话退役了手写 helper**（8bee2515…a28d63ce，`rh_compat`/`test_harness` 都动了），所以先在旧 pin `1f86bbc` 上重测**今天的**脚本作基线：server-smoke 22 128 041 / 527 / 890 920，wake-smoke **34 368 342** / 509 / 1 144 943（脚本退役自己挣了 −5%）；新 pin 同脚本：server-smoke 22 122 272 / 527 / 890 280（−0.03%，环境答复短了 640 B），wake-smoke **34 368 342 / 509 / 1 144 943——步数一步不差**。两条旅程都不写位运算，位运算的字节门在产品脚本上零代价。`295a95f`（Math）：server-smoke 22 128 037 / 527 / 890 920（与旧 pin 基线 22 128 041 差 4 步），wake-smoke **34 368 342 / 509 / 1 144 943——又是一步不差**。两条旅程都不点名 `Math`。`ea820ef`（parseInt）：server-smoke 22 128 065 / 527 / 890 920（基线 +24 步，噪声），wake-smoke 34 339 485 / 509 / **1 142 865**——host_bytes 少了 2 078（环境答复短了），步数差归它。两条旅程都不拼 `parseInt`。
