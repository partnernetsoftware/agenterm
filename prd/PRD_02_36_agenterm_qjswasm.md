...
│   ├── 产品脚本手写过的工具，按需求数落地（tinyvm A12）        [~] 2026-08-31 第一批 `c9df46f`：`Array.isArray` · 数组 `indexOf`/`includes` · `concat` · `join`——`rh_compat.qjs` 的 `is_array`/`array_has`、`cu-macos-smoke.qjs` 的 `invoke_arguments`/`path_text` 可退役（脚本本身未改）；第二批 `eb95e42`：`sort([cmp])`——`prune_target_incremental.qjs:317` 与 `check.qjs:683` 的手写归并可退役（引擎的 3.3× 快，1 000 个串 4.2M 步 vs 13.8M）；第三批 `fb1a1a6`：`charCodeAt` / `charAt` / `substring` / 只读 `s[i]`——`rh_compat.qjs:446` 的 `ASCII_PRINTABLE` 表与 `ascii_bytes`、`qualification.qjs:18` 的字符走法、`build-all.qjs:40` 的 `tail_of` 可退役；剩 `JSON.stringify` 的 `space`
