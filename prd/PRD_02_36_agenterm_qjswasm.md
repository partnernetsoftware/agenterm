...
| # | 事 | 为什么（实测） | 做完算什么 |
|---|----|---------------|-----------|
| A1 | **给 `.qjs` 一个真实的生产用途** | 全仓真实 `.qjs` 只有 `scripts/qjs/lib/fleet.qjs` **一个库、零个任务脚本**；真实工作量是 82 个 `.rh` | 至少一个 `.qjs` 任务脚本被产品真的跑（不是测试里跑） |
| A2 | 决定 `.qjs` 与 `.rh` 的关系 | **没有任何 PRD 说 qjswasm 要接管那 82 个脚本**——不写清楚，A1 就没有依据 | PRD 里有一句产品判断：并存 / 逐步迁移 / 只服务新脚本 |
