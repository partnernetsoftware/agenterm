...
**148 passed / 0 ignored**，2026-08-28，上游 rev **`548fbbe`**）；**`.qjs` 已经够得着
...
**`548fbbe`**（`"ab".length`，当前 pin，2026-08-28）。
...
│   ├── 字符串 / 数字方法（trim · toFixed · map …）           [ ] 需要接收者绑定
...
现在最大的单一缺口。`"ab".length` 已经落地（`548fbbe`），而它恰好是这条路上**唯一
不需要绑接收者**的一格——`length` 是个值，不是个方法。所以它做完之后，剩下的全是
同一个问题：

> `"a".trim()` 里的 `trim` 怎么拿到 `"a"`？

今天没有 `this`，也没有把接收者绑进函数值的机制。三条候选（未判决，各自都站得住）：

1. **加 `this`** —— 跟着规范走，但 `this` 一落地，
   `plan/design-arrow-milestone.md` §1 那条「箭头就是函数表达式」的等价立刻作废；
2. **属性读返回一个绑好接收者的闭包** —— 复用刚落地的闭包机制（cell + 环境向量），
   `a.map` 读出来就是个能直接调的值；
3. **调用点特化** —— `a.map(f)` 直接编成 `__arr_map(a, f)`，根本不产生函数值；
   代价是 `let m = a.map; m(f)` 不成立，而那**今天本来就不成立**。

三条的分歧是结构性的、可度量的（边际成本：每加一个方法各要付多少字节），
符合 `.claude/skills/decisive-experiment` 的立项四条。**这是下一个该做的判决，
不是下一段该写的代码。**

规格：上游 `plan/design-method-binding-experiment.md`；实现与结果
`research/method-binding/`。主判据是**斜率**不是截距；主产出之一是判据 ⑥ 那张
**泄漏清单**。

**状态：进行中，不是结论。** 变体 C（调用点特化）四个方法全完成、判据 ① 7/7；
变体 B（绑定闭包）完成 `trim` 与 `map`；**变体 A（`this`）一行未写**。
按那个 skill 的引用资格，**下面这几个数是「进行中的读数」，不得被当作已决结论**：
...
`map` 那一格的差距是结构性的：**C 必须在每个调用点内联整个循环**（没有 prefab 能
`call_indirect`），**B 的循环只存一份**。若 A 给不出更好的斜率，判决树会指向 B——
**但 A 一行都没写，所以这句话不是结论。**

**三条对产品有意义的副产品**，即使 Q1 还没判：

1. **prefab 是能 `call_indirect` 的**（把统一签名的 type index 交给它就行）。
   原来记的「做不到」是错的，那是「要先付入场费」。
2. **方法体是三个变体共享的**，所以真正在被比较的只有绑定机制——
   `trim`/`indexOf`/`push`/`map` 四个体已经写好且经过一份**变体无关**的语料
   （`method_conformance.rs`，写在任何变体之前，三个变体都必须原样通过）。
3. **一条方法论已被这条轨证实三次**并写进 `.claude/skills/decisive-experiment` §2.6：
   **判据第一次难看时，先问「这是变体的性质还是这次实现的性质」，
   修掉再测，然后才走判决树。** 三次都是后者，三次都差一步误判掉一个变体。

研究用的三个 feature 全部 **default-off**，本产品的 pin（`548fbbe`）看不见它们，
所以这条轨到今天为止**没有改变 agenterm-qjswasm 的任何能力状态**。

语法侧还剩的（各自独立、都不挡门）：`switch`/`break`/`continue`/`do`/`for…of`、
位运算与 `**`/`??`/可选链/解构/展开、`class`、正则、十六/八/二进制字面量与数字分隔符、
带标签的模板、以及**参数表**（默认 / rest / 解构）——最后这条箭头和普通函数共用一套，
做一次两边都开。
