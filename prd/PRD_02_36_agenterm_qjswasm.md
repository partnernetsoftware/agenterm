...
上游 rev **`d319bf9`**，即 `Cargo.lock` 里的 pin）；
...
**还没完的**（按值排）：(1) ~~A1.10(o) 门答复的 JSON 信封~~ 已加 `process.command_stdout`（`71ecacd8`）；(2) 步数价格第二层：`__jp_at` 每位一次、短键每串一次
