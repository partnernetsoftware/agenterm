...
│   ├── 取消能打断等待（A1.14）                                     [x] `Budget.cancel` 穿到门；`cancelled` 类别第一次被产生
