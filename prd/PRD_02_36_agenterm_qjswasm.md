...
│   ├── 数学 / 位运算 / 格式化工具，按需求数落地（tinyvm A13）   [~] 2026-08-31 第一批 `2e26e3d`：位运算 `& | ^ ~ << >> >>>` + 六个复合赋值——`rh_compat.qjs` 的 `xor16`（16 轮 `% 2` 循环）与 `fnv1a64_hex` 的 16 位 limb 乘可退役（54× 快）；下游调用数：`int_div` 68 行/8 文件、`int_of` 46/7、`only_chars` 14/6、`rfc3339_from_ms` 10/3、`parse_int` 6/4、`fnv1a64_hex` 2/2（`rfc3339_from_ms` 要 `Date`，B 区不做）
