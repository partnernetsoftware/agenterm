...
│   ├── 产品脚本手写过的工具，按需求数落地（tinyvm A12）        [~] 2026-08-31 第一批 `c9df46f`：`Array.isArray` · 数组 `indexOf`/`includes` · `concat` · `join`——`rh_compat.qjs` 的 `is_array`/`array_has`、`cu-macos-smoke.qjs` 的 `invoke_arguments`/`path_text` 可退役（脚本本身未改）；剩 `sort`、`charCodeAt`/`charAt`/`s[i]`/`substring`、`JSON.stringify` 的 `space`
