# Archived roadmap history through v0.1.16

> Historical ledger only. Current version truth is
> `prd/PRD_02_18_roadmap.md`; do not dispatch work from this snapshot.
...
- [~] M11 / v0.1.12 planned, delivered in v0.1.14 — Convergence and fast
  ...
  - [ ] sequencing, runner experiments, release SLOs, risks and delivery
    ...
    analysis, [v0.1.15](../../plan/archive/plan-v0.1.15.md) for in-flight work; the
...
- [ ] Multi-platform GUI track (independent of v0.1.8–v0.1.10 version gates):
  ...
  [plan-multiplatform-gui.md](../../plan/archive/plan-multiplatform-gui.md). Native
...
- [ ] `agenterm-cu` computer-use track: decision P4 in
  [`plan/archive/plan-v0.1.15.md`](../../plan/archive/plan-v0.1.15.md) §5.6 — whether to start
...
- [~] M15 / Script Runtime migration complete; Portable App layered deployment planned
  ...
  - [x] incremental **Rhai → rh** migration retired the live `.rhai` corpus,
    old PE/REPL and interpreted fallback; current evidence is `plan/archive/plan-rh-3.md`
...
- [ ] ~~(待派单) **wasm/qjs 引擎重构为依赖 tinyvm**~~ — 把 agenterm 内 `agenterm-wasmcore` / `agenterm-qjs` 的处理改为基于独立 tinyvm 运行时（依赖方向 agenterm → tinyvm）。**2026-08-23 记录，等另行下单再动工**；下单前不写代码、不改 Cargo 依赖。关联：tinyvm 独立仓 PRD、`plan/archive/plan-v0.1.16.md` 脚本引擎小节。
