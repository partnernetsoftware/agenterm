# Archived `agenterm-qjswasm` history through v0.1.16

> Historical invention and evidence ledger only. Current contract is
...
上游 rev **`028a914`**，即 `Cargo.lock` 里的 pin）；

> **这一行以前停在 2026-08-28 / rev `33c9ba2` / 153 passed，而 pin 已经走到
> `c2300fc`。** 记在这里而不是悄悄改掉：**一份 PRD 的第一行落后，是它整体可信度的
...
**上游数组落地，两引擎之间最后一条具名分歧消失（2026-08-26，rev `c0d7ae4`）。**
...
**rev `e1122ff → 14a641a`（2026-08-25 上午）带来的判定变化**：语言层挡着归档门 1 的六件
...
| 值表示（64 位标记联合 vs 32 位 NaN-boxing 的取舍理由） | **已判决**：V1 双字 `(tag:i32, payload:i64)`，由实测实验定，见 `plan/archive/design-value-representation-experiment.md` 与 `research/value-representation/RESULTS.md` | 已落地 | **高** |
...
三条门（能力差异实测 / 差什么写成可判 / 现状复核）走完后按需求归档。同一份 wasm 纯计算 2000 万轮 wasmtime 30.1 ms 对 tinyvm 16.08 s（535×）这个数字留在上游 PRD「原生降级」一节当依据。门的逐条记录见 [归档](PRD_02_36_agenterm_qjswasm_history_2026-08.md#归档-agenterm-wasmcore-的门)。
...
正好顶到上限」去猜会把真坏的脚本误判成预算问题），去上游补。上游补了：`e1122ff` 让
...
**为什么值得画。** 2026-08-26 抓到的三条真缺陷全是同一种病：东西放错房间；此后每一课（十一课，2026-08-26→29：并行按共享资源拆、grep 数到散文、`kill` 杀的是进程不是树、一把毒锁十一条陪葬、测试 spawn 的不是自己编的二进制、前提要问不要猜、注释预言自己的未来……）都收在 [归档](PRD_02_36_agenterm_qjswasm_history_2026-08.md#记忆宫殿的十一课)。留在这里的是表：**哪些东西曾被放错、放错在哪、代价多少**——判「该改哪间」时先查它。
...
三波迁移（2026-08-29，每波「一组写、另一人照命令重跑」）的过程记录——五种截断拼法、五个 `wait` 名字、三组独立量出的同一条步数曲线——见 [归档](PRD_02_36_agenterm_qjswasm_history_2026-08.md#三波迁移的过程)；结论在待办 A1.5。
...
> 代价逐条点名在 [PRD 02.10 §What went dark](PRD_02_10_script_runtime_history_through_v0.1.16.md#what-went-dark-on-2026-08-29)：
> **39 条 qualification 门、4 条 host-native 门、71 个任务**，全暗到各自的 `.qjs` 版落地为止。
> 上一段数的「15 条测试、8 个门脚本」是按**文件**数的，暗掉的单位是**门**——差了一倍多，
> 记在记忆宫殿的表里。

与上游 [`tinyvm/prd/PRD.md`「待办清单」](https://github.com/partnernetsoftware/tinyvm) 同一套写法：
...
| A1.16 | **`env.get` 的脱敏类** | grok 评审 §7.4：`env.get` 是裸门，没有脱敏类；一个 `.qjs` 任务能把 `GITHUB_TOKEN` 读出来写进日志 | **已做（2026-08-30 深夜）**：工具 profile 下 `env.get` / `env.has` 对**像密钥的名字**答有类型的拒绝——停放句 `env.get: \`X\` denied (secret): …`（`get` 状态 1、`has` 答 −1），收据行 `tool.env.get(X) denied:secret`；名单（`tool::secret_looking`，大小写不分）：后缀 `_TOKEN` / `_SECRET` / `_KEY` / `_PASSWORD` / `_PASSWD` / `_PASS` / `_CREDENTIALS`，任意位置含 `PASSWORD` / `PASSWD` / `SECRET`，前缀 `AWS_`，以及 `GITHUB_TOKEN` / `GH_TOKEN` / `NPM_TOKEN` / `API_KEY` / `TOKEN` / `PASSWORD` / `SECRET` 本身；`GITHUB_SHA` / `GITHUB_RUN_ID` / `HOME` / `PATH` / `AGENTERM_*` 都在外。放行：任务契约新字段 `env_allow: [NAME]`（`validate_env` 同规则校验）→ `task run` 翻成 `--env-allow NAME`（可重复）→ 协议 `ScriptInvocation.env_allow`（默认空）→ `Budget.env_allow`；放行的读在收据上记名不记值 `tool.env.get(NAME)`，普通名字的收据行不变。仓里 66 个入口脚本读的 43 个名字没有一个撞名单（`env_or("OS")`、`GITHUB_STEP_SUMMARY`、`AGENTERM_BOOTSTRAP_*`…），无需给任何任务加 `env_allow`；`check` 全绿、corpus-scan 全绿。钉：`tests/tool_door.rs::secret_looking_environment_names_are_refused_unless_allowed`、`tool::tests::the_secret_class_is_the_documented_list`。**没做**：沙箱 profile 没有 `env`，不涉及；`env.cwd` 不在类里 |
...
其 16 × 258 并发 journal mutation 有 4,128 次 process start 和同数 wait；首次
...
得出的。上游 rev 链：`85189c7` → `049ebba`（2026-08-24）→ `1271a00`（门的机制）→
`e1122ff`（客人自报堆耗尽）→ `14a641a`（对象 / 函数值 / try / JSON / `?:` / 三个转换）→
`c0d7ae4`（数组，含 JSON 收发）→ `8f73c5c`（Array 出脸时具名）→ `eb9229c`（捕获闭包）→
`05d506e`（整个 DecimalLiteral）→ `3d8ed07`（模板字面量）→ `ff5d2ac`（箭头函数，位置测试补在 `af3eed6`）→
`4f6af7c`（`"ab".length`）→ `130e929`（五个方法）→ `33c9ba2`（每轮新绑定）→ `6c1628c`（`for … of`）→ `a1f76f9`（模块）→ `f3748a9`（includes/startsWith/endsWith）→ `8a15552`（`split`）→ `58a4fb9`（`toLowerCase`）→ `d43701b`（break/continue + replace/replaceAll）→ `6e2e308`（`Number`）→ `c2300fc`（第四类 fault code）→ `25fcf02`（未捕获 throw 的消息指针 `FAULT_THROWN` + `Object.keys` 折叠）→ `9847658`（缺失的 String 属性报自己的名字：`FAULT_MISSING_STRING_METHOD`）→ `b04e2cd`（`String.prototype.slice`，码元位置，两种元数共用一个核心）→ `932243c`（宿主参数类型错在运行期报 `host#n`：`FAULT_HOST_ARGUMENT`）→ `c1b1098`（Number 参数同样）→ `5714dc6`（`slice` 懒长度：非负索引不数全串，`slice(0,10)` 于 1 000 字符 78 000 步 → <3 000）→ `1c47004`（`"" + n` 整数走位数循环：≈5 200 步 → 537，每个程序 +175 B）→ `ef799d9`（`0x`/`0o`/`0b` 字面量；保留字可作属性名 `o.class`）→ `a8725ea`（`JSON.parse`：整数与短小数一趟读、字符串四字节一步）→ `904b041`（`undefined.x`/`null.x` 报键名：`FAULT_PROPERTY_OF_NON_OBJECT`）→ `17a8c64`（`try` 里的 `undefined.x` 是可捕获的 TypeError）→ `c4ec18a`（`s + "x"` 八字节一步：1 000 字符追加 17 178 → 2 569 步，字节钉 +85；下游 CLI 二分复测：1 000 字符追加一字 **+2 562 步**，100 次十字追加建串共 183 525 步）→ `62dd6e3`（`JSON.stringify` 引号串按段复制：1 000 字符 117 → 39 步/字节，50 个小对象 254 311 → 224 381 步，JSON 程序 +83 字节；下游 CLI 二分复测：50 个小对象序列化 1 931 字节 **224 552 步 ≈116/字节**，与进程内一致）→ `1170fc5`（`.length` 八个 ASCII 字节一步：6 000 字符 180 346 → 19 854 步/次，`for (i < s.length)` 的二次常数小 9 倍；能到串 `.length` 臂的程序 +58 字节；PRD 记了 stringify 逐节点常数表；两条旅程在 64M 默认下 PASS，二分复测 server-smoke 34M → **27.4M**、wake-smoke 43.4M 不变）→ `d0eaa97`（闭包调用**自己也捕获**的声明函数不再让编译器 panic——`record_captures` 把被调者的捕获沿作用域链转发到不动点；workbench-smoke 的接线撞出来的）→ `7c0803d`（`let j = JSON` 在有闭包的程序里过装载门：`__json_ns` 在有捕获时给 `__fn_new` 递环境字；同一个脚本再走一步撞出来的更老的洞）→ `4aeb7c6`（`indexOf` / `includes` 跳过没有首字节的四字节窗：128 KiB 未命中 36 → 7.2 步/字符；lint 与 prd-alignment 两道门撞出来的）→ `9e1974f`（`toLowerCase` ASCII 快路 393 → 38）→ `e8f1686`（`split` 同一个窗跳过 73 → 26）→ `efd4134`（A10：调用非函数是有名字的拒绝——可捕获的 `TypeError: <name> is not a function`，或 fault 8 + 名字；本仓 `QjswasmError::NotAFunction`，类别 `script`）→ `6621176`（A11(a)：对象/数组/函数的 ToString **与 ToNumber** 是有名字的拒绝——fault 9 + 种类 `an Object` / `an Array` / `a function`，**不给 `[object Object]`**（引擎原则「从不静默转换」，第一版给了答案被十一条测试反过来）；`__to_number` 里两条遮蔽的无名臂删掉，每个程序 −18 B；本仓 `QjswasmError::NoPrimitiveForm(kind)`，类别 `script`，CLI 面 `the script used an Object where a String or a Number was needed … write JSON.stringify(x)`；同批 rustc 1.97 的 fmt/clippy 漂移单独一提交）→ `16da41d`（A11(b)(c)(d)：脚本走得到的最后四个无名 trap 都有名字——`a["x"] = 1` / `s[0] = "x"` 是 `FAULT_INVALID_WRITE = 10` + 理由，本仓 `QjswasmError::InvalidWrite(what)`，类别 `script`；`split("")` 与代理对中间的 `slice` 仍是能力边界但带名字，`CapabilityBoundary(Option<String>)`；新扫描位 `member_write` 门控，不写成员的程序字节不变）→ `1fc4f45`（宿主 `I32` 参数收到非整数——`sleep_ms(1.5)`——与 String 参数同名 `host#n`；A11 脚本走得到的一半到此全部有名字）→ `02ec304`（`Module::from_bytes_explained`：校验函数体失败时答出是哪个函数——索引 + `name` 段里的名字；本仓 `QjswasmError::LoadInFunction`，CLI 面 `loading wasm: validation: type mismatch in function \`broken\` (#1)`，A7 那种二分 900 行的事不再需要；静态核字节不变 101 256）→ `59870fe`（被拒的函数还答出**源码行**：编译器在 `name` 段旁写 `qjs.lines` 自定义段（函数索引 → 1-based 行，词法器按 ECMA-262 LineTerminator 数，`import` 的模块按自己的源码数），`FunctionSite.line`，本仓 `LoadInFunction { line }`，CLI 面 `… in function \`broken\` (#1) (line 7)`；脚本本身与运行时函数不列，无函数的程序字节不变、有函数的 +17 起（fleet 库 +89）；静态核字节不变 101 256）→ `77a804b`（`JSON.parse` 第二夜：`__jp_ws` 自己装载、`\n` 后的缩进四空格一字再两个（1 000 空格 39 → 14 步/字节，缩进行 7）；`__json_pstr` 先用新的 `__jp_run` 量整段，段尾是 `"` 就直接 alloc 成记录（`"ab"` 758 → 591）；34 份真实答复 13.56M → 8.45M 步（−38%）；JSON 程序 +353 B，`return 1;` 不动；静态核字节不变 101 256）→ `575c33e`（被拒的函数还答出**列**：`qjs.lines` 每项 `(index, line, column)`，列是行内 1-based 的 UTF-16 码元数（编辑器的数法），CLI 面 `… in function \`broken\` (#1) (line 7, column 12)`；本仓 `LoadInFunction { column }`；无函数的程序字节不变，有函数的每个函数 +1 B（fleet 库 +30）；静态核不变 101 256；2026-08-30 深夜）→ `129ea1d`（`__num_to_string` 的位数循环覆盖整个安全整数区：f64 里拆 `hi·1e9 + lo`（指令集无 i64 除法），13 位毫秒时间戳 `"" + n` **32 786 → 797** 步、`JSON.stringify` 里 32 567 → 541、journal 记录 82k → 51k；每个程序 +191 B（`return 1;` 10 007 → 10 198）；静态核不变 101 256；2026-08-31）→ `5dc2288`（`__obj_find` 第一稿：指针在前、短键掩码——同形键快 3×，键长各异的真实记录反而慢，两条旅程 +0.8% / +1.4%；能持有对象的程序 +158 B）→ `b8319ff`（`__obj_find` 长度在前：长度不同即未命中 28 → 16 步，同长再比指针、首末字、再 `__str_eq`；journal 记录五键各读一次 1 964 → 821、lease 答复 `JSON.parse` 18 083 → 17 815、20 键读末键 2 834 → 952；能持有对象的程序 +119 B，`return 1;` 不动；静态核不变 101 256；2026-08-31）→ `12cfdfe`（产品脚本手写工具第一批：`Array.isArray`（parser 折成 `x.__is_array()`，`rh_compat.qjs:200` 曾手写「有数字 length 的对象」）、数组 `indexOf` / `includes`（与 String 同名，一个 prefab 按 tag 分派，`rh_compat.qjs:215` 的手写 `contains` 可退役；String 调用者 +221 B）、`concat(x[, y])`（`cu-macos-smoke.qjs:277` 的 argv 拼接）、`join([sep])`（`:326` 的路径拼接；undefined/null 为空，对象元素具名拒绝）；价目 59 / 702 / 623 / 569 / 871 B，未命中 52 步/元素、concat 28、join 236/10 字节元素；`return 1;` 不动；两条旅程账单不动（见下 §旅程计分板）；静态核不变 101 256；2026-08-31）→ `e45010b`（第二批：`sort([cmp])`——自底向上归并，稳定、原地、返回接收者，undefined 殿后，默认序 = ToString 后码元序，非函数比较器具名 `comparefn`；1 000 个数 + 比较器 1 708 542 步、1 000 个串默认序 4 207 002 步，同一输入的手写归并（`prune_target_incremental.qjs:317` 原样）13 793 093 步——**3.3×**，那两份手写归并（另一份 `check.qjs:683`）可以退役，脚本未改；948 / 1 116 B；`return 1;` 不动；静态核不变 101 256；2026-08-31）→ `b2b60fb`（第三批：`charCodeAt` / `charAt` / `substring(a[, b])` / 只读 `s[i]`，四个共用 `slice` 的码元核心，核心学会八个 ASCII 字节一步（1 000 字符第 999 位 42.7 → 2.1 步/码元，逐位 `charCodeAt` 全串 23.3M → 3.0M，`slice` 同受益）；`charCodeAt` 落在代理对中间答那半边，`charAt`/`s[i]`/`substring` 落在那里 = `slice` 的同一句拒绝；`s[i]` 的门是「文本定不了是 String 的计算键读」，`o[k]`/`a[i]` 的程序也付 +716 B（记在 `arrays_m3` 那行），步数 `a[i]` 不付；`rh_compat.qjs:446` 的 `ASCII_PRINTABLE`/`ascii_bytes`、`qualification.qjs:18`、`build-all.qjs:40` 的 `tail_of` 可退役，脚本未改；价目 733 / 869 / 1 026 / 909 / 768 B；`return 1;` 不动；静态核不变 101 256；2026-08-31）→ `eeb0cbd`（第四批、最后一批：`JSON.stringify(v, null, space)`——25.5.2 第 5–8 步原样，数 1..10 个空格、串取前 10 码元、其它无缩进；gap 与深度放在序列化缓冲头里，无 gap 时 `__jb_nl` 什么都不写，紧凑答案逐字节不变、价格不动（50 个小对象 178 202 步前后一样，space=2 时 281 965，61/输出字节）；`{}` / `[]` 仍是空、省略的属性不留空行；`replacer` 非空仍具名拒绝（`this engine does not support a JSON.stringify replacer yet`）；`rh_compat.qjs:96` 的 `stringify_pretty` 可以真的 pretty 了，脚本未改；JSON 程序 +501 B；`return 1;` 不动；静态核不变 101 256；2026-08-31）→ `5144a83`（A13 第一批：位运算 `& | ^ ~ << >> >>>` 与 `&= |= ^= <<= >>= >>>=`——ECMA-262 7.1.6 ToInt32 一个 prefab（NaN/±∞→0、截断、模 2^32，全程精确），七个运算符各一个 prefab，扫描按文本点名（精确门）；优先级三级 `| ^ &` 在 `&&` 与 `==` 之间、移位在关系与加法之间；一个运算符 45 步（2^31 外 65）、~170 B，第二个 +48 B；1 000 次 `xor16` 手写 14 526 095 步 vs `(h ^ i) & 0xffff` 267 079（54×）——`rh_compat.qjs` 的 `xor16` 与 `fnv_mul_prime`/`hex4` 的 16 位 limb 路可退役；不写位运算的程序字节不动，`return 1;` 10 198 不动；静态核不变 101 256；2026-08-31）→ `5af14dd`（A13 第二批：`Math` 十个函数 + `PI`/`E`——无 `Math` 对象，每个成员 parser 折成保留方法名（`Math.floor(x)` = `x.__math_floor()`，`Object.keys` 的老路），常量与 `min()`/`max()`/`min(x)` 折成字面量或 `+x` 零字节零步；`round` 平局向 +∞、答 0 带原符号，`min`/`max` = IEEE 2019（NaN 传染、−0<+0），`pow` 特例表 + 整数指数平方求幂，正底分数指数具名拒绝（`a fractional Math.pow exponent`）；缺的成员编译期具名拒绝；floor 270 B / 25 步，`int_div` 手写 633k → `Math.trunc(a/b)` 210k 步（3.0×）——`rh_compat.qjs` 的 `int_div`（下游 68 行/8 文件）与 `floor_div` 可退役；不点名的程序字节不动；静态核不变 101 256；2026-08-31）→ `ab946d6`（A13 第三批：`parseInt(s[, radix])` + `Number.isInteger` / `Number.isNaN`——19.2.5 整条（空白表、一个符号、`0x`、逐位到第一个非位、无位 NaN、基数出 2..36 是 NaN；2^53 外逐位正确舍入，`"9…9"×20` 与 V8 逐位一致）；`isInteger`/`isNaN` 是类型测试不转换；守了一年的拒绝钉 `parse_int_is_not_silently_number` 退役、预登记的 `parse_int_reads_a_prefix_in_the_given_radix` 接班；200 次手写 `parse_int` 2 211 082 步 → 135 279（16×）——`rh_compat.qjs` 的 `parse_int`（下游 6 行/4 文件）可退役；价目 1 073 / 287 / 265 B；不点名的程序字节不动；静态核不变 101 256；2026-08-31）→ **`028a914`**（A13 第四批：`toUpperCase`（小写表反转 + ASCII −32；ς→Σ 手补，ß/µ/四个 titlecase 原样是记录的分歧；8 984 B 两表各自门控）、`padStart`/`padEnd`/`repeat`（StringPad 原样，整倍 + `slice_core` 截前缀，代理对中间是 `slice` 的同一句拒绝；缺省填充一个空格；负 repeat 具名拒绝；1 635 / 1 633 / 669 B）、`Number.prototype.toString(radix)`（逐位除基，V8 同一走法，2^53 外逐位一致；2..36 外具名拒绝；非十进制下的分数拒绝不近似；938 B；`hex4` 可退役）、`toFixed(0..100)`（小 bignum 精确十进制，不是 `%f`：`(1.005).toFixed(2)`="1.00" 而 `(8.005)`="8.01"，都是双精度的真话；平局远离零；1e21 起走 ToString；1 243 B）；`method_conformance` 的缺失成员样例再次换座（`toUpperCase`/`toFixed` → `trimStart`/`toPrecision`）；`return 1;` 与所有字节钉不动；静态核不变 101 256；当前 pin，2026-08-31）。
...
│   ├── 数字字面量：整个 DecimalLiteral 文法                  [x] rev 05d506e
│   │   ├── 1.5 · .5 · 1. · 1e3 · 2E2 · 1.5e-3                [x]
│   │   ├── 超出 i32 / 超出 2^53 的整数                        [x] 取最近 double
│   │   └── 十六 / 八 / 二进制 / 数字分隔符                     [~] `0x`/`0o`/`0b` 已落地（上游 ef799d9，`tests/radix_literals.rs`）；数字分隔符 `1_000` 未做
│   ├── 模板字面量                                          [x] rev 3d8ed07
│   │   ├── `` `abc` `` / `` `a${x}b` `` / 任意嵌套            [x]
...
│   ├── 方法：trim · indexOf · push · pop · map              [x] rev 130e929
│   ├── includes · startsWith · endsWith                     [x] rev f3748a9，需求普查前两名
│   │   ├── 字节层比较，对 é / 😀 这类多字节字符精确         [x] UTF-8 自同步，是性质不是假设
│   │   └── includes 不经由 indexOf：320 vs 440 字节         [x] 布尔值没有位置要报告
│   ├── split（非空分隔符）                                  [x] rev 8a15552，426 字节
│   │   └── split("") **trap**：孤立代理 UTF-8 表示不了       [具名] 表示层禁止，不是未实现
│   ├── toLowerCase（Unicode 区间表）                        [x] rev 58a4fb9，**8 836 字节**
│   │   ├── 价目公开：用到它的脚本约 +90%                     [x] 判据 ④ 记录、不设上限
│   │   ├── 中文 / emoji / 已小写原样返回，不 trap            [x] 判据 ②
│   │   └── `İ` 一对多、词尾 `Σ` → `ς`                        [具名] 两条分歧
│   ├── toUpperCase                                          [–] 语料 67 次里零使用
│   ├── replace（首个）/ replaceAll（全部）                   [x] rev d43701b，语料写前者意思是后者
│   ├── `break` / `continue`（无标签）                        [x] rev d43701b
│   │   └── 跨 `finally` 的                                   [ ] 具名拒绝，要 pending 机械
│   ├── `Number(x)`：折成 `+x`，零运行时                      [x] rev 6e2e308，缺的只是名字
│   │   └── `parseInt`（前缀 + 基数，与 Number 不同）         [ ] 按名等需求，不做别名
│   ├── 需求普查：**已无「有需求且未做」的行**                [x] 剩下的全是零使用
│   └── String 上非 `length` 的属性读取                       [x] **决定对，诊断补上了**
│       ├── 第四类 fault code + 宿主面一句人话                 [x] rev c2300fc，上游 7 字节
│       └── 跨 `finally` 的 break / continue                   [–] 数完决定不做：语料 1 处
│   ├── 循环里每轮是一个新绑定（14.3.1 / 13.7.4.7）          [x] rev 33c9ba2
│   ├── `for … of` 遍历数组（13.7.5）                        [x] rev 38bd560，需求第一
│   ├── 模块：`import * as` + `export`（16.2）               [x] rev a1f76f9，需求第二
...
│   ├── 产品脚本手写过的工具，按需求数落地（tinyvm A12）        [x] 2026-08-31 第一批 `12cfdfe`：`Array.isArray` · 数组 `indexOf`/`includes` · `concat` · `join`——`rh_compat.qjs` 的 `is_array`/`array_has`、`cu-macos-smoke.qjs` 的 `invoke_arguments`/`path_text` 可退役（脚本本身未改）；第二批 `e45010b`：`sort([cmp])`——`prune_target_incremental.qjs:317` 与 `check.qjs:683` 的手写归并可退役（引擎的 3.3× 快，1 000 个串 4.2M 步 vs 13.8M）；第三批 `b2b60fb`：`charCodeAt` / `charAt` / `substring` / 只读 `s[i]`——`rh_compat.qjs:446` 的 `ASCII_PRINTABLE` 表与 `ascii_bytes`、`qualification.qjs:18` 的字符走法、`build-all.qjs:40` 的 `tail_of` 可退役；第四批 `eeb0cbd`：`JSON.stringify(v, null, space)`——`rh_compat.qjs:96` 的 `stringify_pretty` 可以真的 pretty；`replacer` 仍具名拒绝。**四批全部落地**；**脚本侧退役 2026-08-31 落地**（`plan/design-host-op-budget.md` §8）：`rh_compat` 的 `is_array`/`is_map`/`array_has`/`list_has_value`/`sort_strings`/`ascii_bytes`（`ASCII_PRINTABLE` 表删除）/`stringify_pretty`（真 pretty）改走引擎，两份手写排序、`invoke_arguments`/`path_text`、`tail_of` 退役，导出名与调用点不变；`only_chars` **留下**——量过 `replaceAll` 删字比逐位 `charCodeAt` 便宜（64 hex 52k 对 93k–113k 步，每次字符访问一次 prefab 调用）；三条旅程 wake −5.0%（轮询里的 `array_has`）、server −0.1%、cu −0.13%，STEP/EVIDENCE 不变
│   ├── 数学 / 位运算 / 格式化工具，按需求数落地（tinyvm A13）   [~] 2026-08-31 第一批 `5144a83`：位运算 `& | ^ ~ << >> >>>` + 六个复合赋值——`rh_compat.qjs` 的 `xor16`（16 轮 `% 2` 循环）与 `fnv1a64_hex` 的 16 位 limb 乘可退役（54× 快）；下游调用数：`int_div` 68 行/8 文件、`int_of` 46/7、`only_chars` 14/6、`rfc3339_from_ms` 10/3、`parse_int` 6/4、`fnv1a64_hex` 2/2（`rfc3339_from_ms` 要 `Date`，B 区不做）；第二批 `5af14dd`：`Math` 十个函数 + `PI`/`E`——`int_div`/`floor_div` 可退役（`Math.trunc(a/b)` 3.0× 快，缺员编译期报名、分数指数具名拒绝）；第三批 `ab946d6`：`parseInt` + `Number.isInteger`/`isNaN`——`parse_int` 可退役（16× 快）
│   ├── 其它方法（toUpperCase · toFixed · filter）            [ ] 字符串读报名（`MissingStringMethod`）；数组读为 undefined，调用它报名（`NotAFunction`）
...
│   │   └── 未捕获 throw 的 String 宿主可读并打印                 [x] 25fcf02（上游指针）+ 2cde8b63（下游读）
...
│   │   ├── 对象 / 属性 / 函数值 / try / JSON / `?:`           [x] rev 14a641a
│   │   ├── 数组：字面量 / `a[i]` / `.length` / JSON 收发       [x] rev c0d7ae4
...
│   ├── 71 个 .rh 脚本 → .qjs                                  [~] 入口 70/71（只剩 `script-http-fixture`，B 区决定不迁），库 14/11（2026-08-29 第三波：7 组全部合入，corpus-scan 83/83 ok；失败集 = 基线 31；macOS 端到端 PASS：server-smoke、wake-smoke，其余 8 条的停点记在 A1.5）
...
│   │   └── 27 个未证入口按原因分桶补证                              [ ] Windows 产物 / dist / 任务表 / 步数（slice 已落地 b04e2cd）
...
│   ├── `env.get` 脱敏类（A1.16）                                     [x] 像密钥的名字在工具 profile 下拒绝，契约 `env_allow` 放行并记名上收据
│   ├── 宿主操作 / 等待节拍预算（A1.12）                            [x] 计数+上限+账单到 CLI（成功与失败都有）；两条旅程：等待仅 8%，步数是真算；剖面：parse 48% / stringify 28%；parse 砍 38% 后旅程 −16%；`tool_result` 不计操作后 host_ops 1 136 → 530；13 位时间戳 32 786 → 797 步后旅程再 −12% / −8%；`__obj_find` 长度在前再 −1.7% / −1.6%；workbench 到停点 25.1M
...
│   ├── 带 span 的编译期错误（A7 那类 validation）                   [x] 函数名 + 源码行（`LoadInFunction { line }`，`qjs.lines` 段，pin 59870fe）；`.qjs` 前端自己的语法错误早就带字节偏移
...
M0、编译器迁出、抬 rev 到 `049ebba`、接缝对抗审查、门落地、数组落地——六次同机同工具链的复测与两处「源码看着支持、编出来不支持」的记录，见 [归档](PRD_02_36_agenterm_qjswasm_history_2026-08.md#历次复测)。**现行口径**只认「验证口径」一节的三条命令与「自验」表。

## 接下来（2026-08-28 的决策日志，已归档）

01 迁移两处生产调用点 / 归档 `agenterm-qjs`（已完成）、02 报告约定「同一份字节两个引擎同一个答案」、门 2 的同客人性能对比、门 1 判定、「不归档 wasmcore」的决定与随后按需求归档、「默认最优为什么不能自动」、「留着花多少钱：默认构建零」、产品面复验——全部落地或判决，过程见 [归档](PRD_02_36_agenterm_qjswasm_history_2026-08.md#接下来按解锁什么排不按工作量排)。现行的「接下来」= 待办清单 A 区。
...
上游现在让缺失的属性报自己的名字（tinyvm `9847658`），CLI 面写着
...
**同一条命令、同一个二进制的账单，按 pin 记**（`--profile tool --max-operations 1000000000`，`steps / host_ops / host_bytes / waited_ms / heap_pages`；步数随环境答复长度有 0.01%–0.1% 噪声）：`b8319ff`：server-smoke **22 517 385** / 527 / 889 561 / 281 / 32，wake-smoke **37 221 773** / 509 / 1 144 943 / 246 / 43。`12cfdfe`（isArray · 数组 indexOf/includes · concat · join）：server-smoke 22 528 222 / 527 / 890 920 / 279 / 32，wake-smoke 37 221 777 / 509 / 1 144 943 / 261 / 43——**两条旅程都不点名这些方法，账单在噪声内不动**（+0.05% / +4 步），这就是「不点名不付钱」在产品脚本上的证据。`e45010b`（sort）：server-smoke 22 475 710 / 527 / 885 699 / 279 / 32，wake-smoke 37 222 633 / 509 / 1 143 904 / 244 / 43——不动（−0.2% / +0.002%，前者是环境答复短了 3.9 KB）。`b2b60fb`（字符访问）：server-smoke **22 116 744** / 527 / 890 201 / 279 / 32，wake-smoke **36 151 626** / 509 / 1 144 943 / 260 / 43——**降了 1.6% / 2.9%**，不是噪声：两条旅程都不点名新方法，但都点名 `slice`（server-smoke 1 处、wake-smoke 2 处、`test_harness.qjs` 2 处），而 `slice` 的码元核心这一批学会了八个 ASCII 字节一步。`eeb0cbd`（`space`）：server-smoke 22 147 642 / 527 / 890 201 / 276 / 32，wake-smoke 36 178 917 / 509 / 1 144 943 / 242 / 43——+0.14% / +0.08%，在噪声带内（紧凑路每个属性多读一次 gap 字，是这一批唯一落在旅程上的东西）。**四批合计**：server-smoke 22 517 385 → 22 147 642（−1.6%），wake-smoke 37 221 773 → 36 178 917（−2.8%）——两条旅程没点名任何新方法，账单没涨，降的那截是 `slice` 共享的走法。`5144a83`（位运算）：**脚本在两个 pin 之间被另一会话退役了手写 helper**（8bee2515…a28d63ce，`rh_compat`/`test_harness` 都动了），所以先在旧 pin `eeb0cbd` 上重测**今天的**脚本作基线：server-smoke 22 128 041 / 527 / 890 920，wake-smoke **34 368 342** / 509 / 1 144 943（脚本退役自己挣了 −5%）；新 pin 同脚本：server-smoke 22 122 272 / 527 / 890 280（−0.03%，环境答复短了 640 B），wake-smoke **34 368 342 / 509 / 1 144 943——步数一步不差**。两条旅程都不写位运算，位运算的字节门在产品脚本上零代价。`5af14dd`（Math）：server-smoke 22 128 037 / 527 / 890 920（与旧 pin 基线 22 128 041 差 4 步），wake-smoke **34 368 342 / 509 / 1 144 943——又是一步不差**。两条旅程都不点名 `Math`。`ab946d6`（parseInt）：server-smoke 22 128 065 / 527 / 890 920（基线 +24 步，噪声），wake-smoke 34 339 485 / 509 / **1 142 865**——host_bytes 少了 2 078（环境答复短了），步数差归它。两条旅程都不拼 `parseInt`。

**引擎侧**同日上游十个提交，pin 跟到 `efd4134`（后四个：`indexOf`/`includes` 窗跳过、`toLowerCase` ASCII、`split` 窗跳过、**A10 调用非函数有名字**、**A11(a) 对象/数组/函数无原始形式有名字**、**A11(b)(c)(d) 拒绝的写与两条表示边界有名字**（pin 随后跟到 `6621176` → `16da41d`）——grok 评审排第一的项，CLI 面 `the script called \`concat\`, which is not a function (TypeError)`，可捕获时脚本自己拿到 `TypeError: concat is not a function`）：三层价格（拼接 6.7×、引号串 3×、`.length` 9×）、两处「第一个真脚本」照出的老洞（被调者捕获转发、`JSON` 当值配闭包）。上游三条腿 1043/0、315/0、iOS exit 0；本仓 package 197/0、lib 720/2（平台对）、routing 14/0、全仓 118 行 31 败（逐名同基线）。
...
| 行 | 状态 | 证据 |
|---|---|---|
| A1.5 迁 71 个入口 + 11 个库 | **70/71 入口、14 个库**；一条**不迁**是决定不是欠账：`script-http-fixture`（要 TCP 门，B）；`cu-windows-smoke` 已于 2026-08-30 迁入（`scripts/qjs/cu-windows-smoke.qjs`） | `ls scripts/qjs/*.qjs` = 71（含 qjs 自有入口）；`corpus-scan` 全 ok；三波每组各有一名核验者照命令重跑，全部「复现」 |
...
| A1.9 引擎缺口 (a)–(i) | (a)(b)(c)(d)(e)(f)(g)(h) 已处理；(i) 无 net 门 = B | 上游 pin 从 `c2300fc` 走到 `904b041`（10 次），每次同一提交改版本链，门响过一次 |
...
**还没完的**（按值排）：(1) ~~A1.10(o) 门答复的 JSON 信封~~ 已加 `process.command_stdout`（`71ecacd8`）；(2) ~~步数价格第二层~~ 今天走了三层（拼接按字 6.7×、引号串按段 3×、`.length` 按字 9×，pin `1170fc5`），剩下的是逐节点常数（上游 PRD A9 有表）；(3) WebKit 差分的浏览器半边要有人在桌面上点一次 Chrome：`__jp_at` 每位一次、短键每串一次
buffer、~~`s + "x"` 的二次拷贝~~ 复制已按字（`c4ec18a`，6.7×；就地追加因不可变性不做）；(3) ~~`undefined.x` 可捕获~~ 已做（`17a8c64`）；
