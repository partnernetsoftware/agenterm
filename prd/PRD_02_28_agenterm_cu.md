...
- [~] Production compatibility is now the embedded Bun-free `skills/acu/acu.qjs`
  ...
  │  │        deadline remain authoritative over a late token; `ps` CPU sampling checks before
  │  │        process inventory authority and slices only the interval between its two identity-bound samples;
  ...
  │  │     focused handle, node name and caller-owned ready marker no longer inherit the
  │  │     window-bound observe guard; the catalog names the same five native conditions
...
- [~] File/storage replacement is classified separately from qjswasm's basic
  ...
  debt, so `resource.volumes` stays `platform-limited`.
  The archived `resource volumes` spelling now reaches this same provider with
...
- [~] Global pointer movement has a registered macOS owned-fixture court with
  ...
  release binary required the v0.1.17 Windows reference to move from
  4 MiB to 8 MiB; the owner suspended the release-blocking size comparison
  while retaining exact Candidate byte-count evidence. The capability ledger
  stays `platform-limited` for the unqualified cell.
