...
│  │     network-probe worker/fixture + managed-job/browser-session/device-lease owners are green,
│  │     but six families keep G1 red; owner lifecycle evidence remains platform-scoped
...
the Bun-free compatibility path preserves the legacy seconds units and its
...
only that mode emits the evidence. Report-mode evidence proves that the
blockers were measured, never that MCU may be removed.
...
`acu.dynamic.075.profile-name-binding` gap. Its frozen alias-trap research
budget ended after two validity failures without an A0/A1/B verdict, so the
TODO remains. Any continuation requires a new precommitment that proves the
stopped browser child owns the bridge connection process identity and retains
a bounded failure stage. Linux/Windows reload courts are still pending.
...
The power branch now has a real plan-only ACU shape instead of an alert with no
