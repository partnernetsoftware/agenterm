...
| 日期 | 决定 |
|------|------|
| 2026-09-01 | **Candidate `33518135806`（`e595b34a`）越过已修复的 qjswasm sleep、release cleanup 与 qualification scratch 边界后，被 PowerShell migration gate 正确拒绝：`examples/powershell/agenterm_uia_fixture.ps1`。** 它是新启用 Windows CU journey 的 WinForms fixture，不是发布自动化，但仓库合同明确规定测试也没有 `.ps1` 例外。不能削弱零文件门禁：fixture 逻辑迁到 `examples/csharp/agenterm_uia_fixture.cs`，既有 `.qjs` journey 直接调用 Windows 自带 .NET Framework `csc.exe` 编译；保留真实 Win32/UIA 控件、不要求外部 C 工具链，也不恢复 PowerShell 源码或运行时依赖。可复用的顺序教训：tracked source topology 变化后，先跑便宜的 migration audit，再进入昂贵 Windows qualification。 |
