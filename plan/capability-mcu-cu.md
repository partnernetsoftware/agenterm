...
| 进程/PTY/job/设备/提权/Simulator/spaces | MCU 工坊/库房/地库 | `ps` 基础 inventory、`process-state` 稳定身份、`process-usage` 无损累计指标已 live；`pty`/`job`/`process` 其余形状仍 **typed unsupported** | 已开始共用 platform/qjswasm 内核；其余属于 ACU 替代门的待迁移 facade，不是永久留 MCU |
