...
| 进程/文件/PTY/job/设备/提权/Simulator/spaces | MCU 工坊/库房/地库 | `ps` 基础 inventory、`process-state` 稳定身份、`process-usage` 单次/有界序列、`process-wait` 原生稳定对象等待、`process-watch` identity-bound lifecycle diff 已 live；`file-inspect` / MCU-shaped `acu file inspect` 已有末级链接不跟随、稳定对象身份与无损元数据（macOS qjswasm 41 STEP / 42 evidence live，L/W 原生 focused court 已绿）；`pty`/`job`/`process` 其余形状仍 **typed unsupported** | 已开始共用 platform/qjswasm 内核；其余属于 ACU 替代门的待迁移 facade，不是永久留 MCU |
