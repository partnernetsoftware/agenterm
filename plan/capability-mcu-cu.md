...
| 后台 tab | `browser tabs`（扩展） | **`browser-tabs [--profile-instance-id ID \| --connection-id ID] [--match SUB \| --tab-id N]`**：MV3 profile-wide 完整清单，保留后台 tab 的稳定 id/window id/title/URL，截断或连接歧义 typed fail；`tab-list --window H` 仅作单窗口可见 tab-strip，不冒充 profile 清单 | **macOS owned-profile qjswasm court 已接线；Linux/Windows court 待升格** |
...
| 开箱 | `setup`/`doctor`/`caps`/`permissions` | `permissions` 是一级 observe-only 命令，并与 `doctor` / `capabilities` 共用同一声明：授权状态 + 修复路径 + 被授权卡住的动词；MCU-shaped `acu permissions` 已直达该命令。`setup` 仍 typed gap，系统同意仍由用户控制 | **状态报告已对齐**；修复向导是 ACU migration gap |
