...
| 进程/PTY/job/设备/提权/Simulator/spaces | MCU 工坊/库房/地库 | `ps` 基础 inventory、`process-state` 稳定身份、`process-usage` 无损累计指标、`process-wait` 原生稳定对象等待已 live；`pty`/`job`/`process` 其余形状仍 **typed unsupported** | 已开始共用 platform/qjswasm 内核；其余属于 ACU 替代门的待迁移 facade，不是永久留 MCU |
...
7. [✓] `page-js` CDP Runtime.evaluate（`--port`，默认 9222）；无 listener typed；禁 MAIN Function constructor。
   `--target-id` / `--target-url` / `--target-title` 选 tab（零命中 `cdp_target_not_found`、多命中 `cdp_target_ambiguous`）；
