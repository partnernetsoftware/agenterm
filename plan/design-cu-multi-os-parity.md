...
| ~~Windows **真机证据**~~ **已有注册旅程** | `cu-windows-smoke` **11 STEP / 11 EVIDENCE**，Windows on ARM 上连跑两次通过（见 §8）。之前三版这里依次写过「没有那台机器」「机器有、进不去」「跑了几个动词」——前两句都是错的，第三句不够。入口在独立 `utm-court` 仓的 `bin/utm-court`（适配器是 **QEMU guest agent，走 virtio-serial 不走 TCP**，所以扫端口注定一无所获）|
...
| ~~macOS `screenshot`~~ **已做** | 之前写「系统拿走的」。**前半句对，后半句是我没查就推的**：SDK 里确实没了（编译器直接拒绝），但**符号还在框架二进制里**——而本仓早就用 dlsym 去够 SkyLight 私有框架。实测 macOS 26.5：符号 present，而且**真能抓**（一个活窗口回来 1120x944 32bpp、17184 段颜色，不是废弃桩该给的 NULL）。于是照 SkyLight 的老办法 dlsym 拿来用，BGRA 转契约的 XRGB，**按 `bytesPerRow` 读而不是假设 `width*4`**（CoreGraphics 会补齐行距，假设就会把图切斜）。代价明写不藏：废弃符号随时可能真的消失，所以**没有它就是点名 ScreenCaptureKit 的 typed 拒绝**，不崩、也不退化成整屏抓图；抓不到窗口也拒绝——这个 API 把「没权限」和「窗口没了」报成同一种，所以消息把两种都说出来。顺带修了 cu 里两处过期的报告：`verbs.screenshot` 在 macOS 上是写死的 `unsupported`，`permissions.screen_recording` 说 grant `not_required`。现在前者跟着机制走，后者如实说「需要」并给修复路径，同时**不假装知道**是否已授予（这个 API 没法把它和「窗口没了」分开） |
| ~~MCU 叶子 `unknown command`~~ **已做** | `dclick`/`rclick`/`shot`/`type`/`key`/`move`/`elements`/`launch`/`quit`/`hide`/`show`/`clipboard`/`page read --js` 是 live 别名；`drag`/`inspect`/`page targets`/`ps`/`maximize` 等 MCU 命令名 typed 拒绝，不再静默 unknown |
| ~~剪贴板富内容读取~~ **已做** | `clipboard-read --type T`：macOS AppleScript class / Linux TARGETS atom / Windows CF_* 或注册名；16 MiB 上限；JSON 带 sha256；`--out` 写文件不把 payload 塞进 stdout |
