...
| **M3 字符串** | guest 侧运行时第一块：线性内存串表示 + bump 分配器。字符串字面量、拼接、比较 | **✅ 已落地**：字面量池 + 拼接 + 相等 + bump 分配器，**三个 ECMA-262 转换全部实现**（`ba143c5`：Number::toString 用 Dragon4、StringToNumber 全文法、按 UTF-16 码元的关系比较）。**上一版说这三条未实现，已作废** | — |
| **M4 堆对象** | 数组 + 对象字面量 + 属性读写。堆布局 + 属性查找 | **✅ 已落地**：对象是 V1 第六个 tag + 扁平 `[key][tag][payload]` 记录（`f203858`），数组是第八个 tag + 密集向量（`048bcf2`），属性读写走 `__prop_get`/`__prop_set` 分派。**上一版说未开工，已作废** | — |
| **M5 parity** | 闭包 + 函数值 + `try`/`catch` + `JSON`。**跑通 `fleet.js` 等价物 → 触发归档门评估** | **✅ 大部分落地**：函数值（第七个 tag + 每次求值一条记录，`ba143c5`）、`try`/`catch`/`finally` + `throw`（`5bdb557`，不用 wasm 异常，是一个 flag 加三个 global）、`JSON`（`90b8aca`/`4fd101d`，是一个持两个函数值的对象不是编译器内建）全部在。**仍缺：捕获外层局部变量的闭包。** `fleet.js` 等价物已跑通，归档门 1 已绿 | ✅ |
