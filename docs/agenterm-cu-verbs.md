...
| `pty-snapshot` | observe | terminal | `pty snapshot` | snapshot one durable PTY job with an event cursor |
...
Returns the sole durable job tab's bounded structured screen plus the exact server epoch and ordered event cursor. The job name, logical server scope, epoch and stable tab id remain one identity; an epoch change during capture fails typed rather than publishing a substituted snapshot. Continue with pty-events.
```
