...
| `tab-close` | actuate | browser | `tab close` | close one tab through its own close button (gated) |
...
```text
tab-close --window HANDLE --title T --exact --expect gone
...
arguments:
  --window HANDLE               window handle from `windows` (numeric or App#N)
  --title T                     the tab's exact title (from tab-list)
  --exact                       required: the title is matched exactly, never as a substring
  --expect gone                 checkable postcondition: the title reads back as absent from the strip

Destructive, so gated like close: --window H --title T --exact names the
