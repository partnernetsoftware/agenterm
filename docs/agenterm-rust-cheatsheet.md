...
- Before projecting a native floating-point result into JSON, reject NaN and both infinities with a stable typed error. Do not rely on a generic JSON value macro: a non-finite `f64` can silently render as `null`, changing a native numeric result into a different valid JSON type.
