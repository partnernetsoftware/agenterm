...
## 2.6 走判决树之前，先做一次「可修性」检查

> **为什么单列。** `~/repos/tinyvm/research/method-binding/` 这条轨上，
> **同一个检查在两天内救下两个变体**——不是巧合，是这套流程缺的一节。

一个判据第一次给出难看的数时，**先问一句**：

> **这个数是「变体的性质」，还是「这次实现的性质」？**

两次实证：

1. **判据「加一个方法的边际字节」= +307**，本该按「线性且陡」直接判负。
   原因是那一版的门是**整集合一刀切**——而那是实现的选择，不是变体的定义。
   改成逐方法门控后，边际变成 **0**。
2. **布尔门「不用这个特性的程序 Δ = 0」不过，多付 514 字节**，本该判负。
   原因是那一版把新臂挂在了一个**别的特性已经打开**的分支上。
   把门分开之后，全部回到 **0**。

两次都是「这次实现的性质」，两次都差一步就把一个变体误判掉。

**做法**：

- 难看的数出现时，**先写下它**，并同时写下「候选解释：变体 / 实现」。
- 若怀疑是实现，**把它修掉再测一次**，两个数都留在 §8 里——
  「先读成 X，修完是 Y」比只留 Y 有信息量得多。
- **修改本身的成本要算在那个变体头上**（多少行、引入了什么新概念），
  因为别的变体不需要它。这样修好也不是白修。
- 若确认是变体的性质，那时才走判决树。

**这条不是「对变体手下留情」**：它要求的是把「可修」这件事**证明出来**（改了、重测了、
数字变了），而不是嘴上说「这个应该能优化」。没有重测过的「应该能修」不算数，
照旧判负。

### 2.6.1 一个变体推翻共同前提时，**所有**变体都要重测

可修性检查不只是「对自己那个变体做一次」。**变体之间会互相改变对方的数字。**

实证（同一条轨，第三次触发，而且**直接改变了判决**）：变体 C 的某个判据读成
另一个变体的 **5.4 倍**，眼看要按「线性且陡」判负。C 之所以那么贵，是因为它绕开了
一条**共同前提**——「这一层做不到 X」。而实现变体 B 的过程**证明了这一层做得到 X**。
给 C 也换成那个做法之后，那个数从 162 降到 48，**判决从 B 翻到 C**。

**做法**：把每条「共同前提」显式写进 §0 或 §1。任何变体在实现过程中推翻其中一条，
**立刻把所有已测变体重跑一遍**，并在 §8 里点名「哪条前提被谁推翻、谁的数字因此变了」。

**为什么值得单列**：先做的变体往往是在**旧前提**下设计的，后做的变体推翻前提之后，
先做的那个就背着一笔本不该背的成本。不重测，等于用「谁先被实现」决定判决。
...
走树之前先过 §2.6 的可修性检查——本仓两次误判都发生在这一步缺席时。
