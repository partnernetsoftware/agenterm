...
if [ "$TASK" = cu-windows-smoke ] || [ "$TASK" = cu-windows-external-terminal-send-smoke ]; then
  ...
  if [ "$TASK" = cu-windows-external-terminal-send-smoke ]; then
    cp "$REPO_ROOT/scripts/qjs/cu-windows-smoke.qjs" "$PAYLOAD/scripts/qjs/"
  fi
fi
...
case "$COURT" in
  ...
  lnx-x86_64-desktop)
    ...
    ;;
  win-aarch64-desktop)
    # A restored Windows desktop can expose QGA before the login-session agent
    # is ready. The same exact nonce probe failed at 180 seconds and then
    # succeeded, out of its Job object, under this bounded recovery budget.
