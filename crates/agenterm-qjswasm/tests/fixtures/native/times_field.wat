(module
  (import "agenterm" "native_call"
    (func $native_call (param i32 i32 i32 i32) (result i32)))
  (memory 1)
  (data (i32.const 0) "|times|i64(ptr)")
  (func (export "main") (param $field i64) (result i64)
    (i32.store (i32.const 128) (i32.const 1))
    (i32.store (i32.const 132) (i32.const 1))
    (i64.store (i32.const 136) (i64.const 0))
    (i32.store (i32.const 144) (i32.const 1))
    (i32.store (i32.const 148) (i32.const 0))
    (i64.store (i32.const 152) (i64.const 137438954496))
    (drop (call $native_call
      (i32.const 0) (i32.const 15) (i32.const 128) (i32.const 32)))
    (if (result i64) (i64.eq (local.get $field) (i64.const 0))
      (then (i64.load (i32.const 136)))
      (else
        (i64.load
          (i32.add
            (i32.const 1016)
            (i32.mul (i32.wrap_i64 (local.get $field)) (i32.const 8))))))))
