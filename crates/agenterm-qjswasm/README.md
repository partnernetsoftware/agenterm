...
当前 pin 已前进到 `9420045`。下面逐项标明的 bitwise 与 `for...of` 又经产品入口复测；
...
- **数字是 ECMA-262 binary64**，不是 i32：`1/0` 是 `Infinity`、`0/0` 是 `NaN`、
  ...
  这些非有限值可在 guest 内照常计算；若把它们直接作为 completion value 返回，产品的
  JSON 结果线会以 `qjswasm_result_not_json` 具名拒绝，而不是冒充 `null` 或“没有值”。
  ...
  **上一版说「字面量还不能写小数」，已作废。**
  **别的进制也已在当前 pin 的产品入口复测**：`0x10` / `0o17` / `0b101`
  分别是 16 / 15 / 5。仍撞具名诊断的是数字分隔符 `1_000`，不要从进制前缀已支持
  外推整套 NumericLiteral 都已落地。
...
- **函数**：声明式带参数、递归与互递归、嵌套声明、读模块顶层绑定。
  ...
  **上一版说「仍然拒绝」，已作废。**
- **运算符**：赋值与复合赋值、`||`、`&&`、`??`（只有 `null` / `undefined` 取右侧；
  `0` / `""` / `false` 保留左侧）、`==`/`!=`/`===`/`!==`、`<` `<=` `>` `>=`、
  `+` `-`、`*` `/`、`%`、`typeof`、前后缀 `++`/`--`、一元 `+ - !`、括号、**`?:`**，
...
| `fs_read_tail_lossy(p,max_bytes)` | `tool.fs.read_tail_lossy` | 只读最后 `max_bytes` 个原始字节，再按 `process.command` 的 lossy UTF-8 规则投影；原始窗口与桥接结果分别限界 |
| `fs_read_lines_starting_with_lossy(p,prefix,max_output_bytes,max_scan_ms)` | `tool.fs.read_lines_starting_with_lossy` | 固定缓冲扫描全文件，只返回原始字节以前缀开头的行；输出超限整体拒绝，逐块观察取消与墙钟截止，`0` 表示无局部截止 |
...
| `fs_append_existing_durable(p, text)` | `tool.fs.append_existing_durable` | 只追加**已存在**的 regular 文件；missing 不创建，逐级 no-follow（拒绝 final/ancestor 符号链接/reparse），同一 opened file object/handle `sync_all` 落盘。**error-after-effect**：`write_all`/`sync_all` 失败时 append 可能部分或完整发生且耐久性未证，绝不创建/删除/截断/替换；调用者须 fail closed 且不得重放同一 record。与 `fs_append` 不同：后者保持 create/follow/non-durable |
| `fs_create_new_regular_durable(p, text)` | `tool.fs.create_new_regular_durable` | **独占新建** regular 文件（`O_EXCL`/`FILE_CREATE`）；existing file/dir/FIFO/socket 与 final/ancestor 符号链接/reparse 全部拒绝且原对象不变，missing ancestor 拒绝。逐级 no-follow，同一 opened file object/handle `sync_all`；Windows 上含 `:` 的名字（`name:stream`/`name::$DATA`）在任何创建前拒绝，避免对既有 base file 新增 ADS。**durability 只覆盖该文件对象的内容/metadata，不覆盖 parent directory entry**（不做 parent fsync）。**error-after-effect**：create 成功后 `write_all`/`sync_all`/类型校验失败可能留下本调用者刚建的 partial/empty 文件，错误不证明 absence/durability，door 绝不删除，调用者须 fail closed 且不得盲重放。Unix mode `0600`；Windows 仅继承父目录 ACL，parent 私密性是**调用者前置**（door 不测量/不后置断言）。与 `fs_write` 不同：后者保持 create/truncate/follow 行为 |
...
| `process_spawn_frozen(spec_json)` | `tool.process.spawn_frozen` | status；与 `process_spawn` 同一 spec，但 child 以**挂起**方式创建：**恢复前**（即目标 executable 的**第一条用户指令前**）先冻结其精确原生 identity，再建立 containment，最后才恢复（macOS `posix_spawn`+`START_SUSPENDED`→freeze→`SIGCONT`；Windows `CREATE_SUSPENDED` 窗口内 `GetProcessTimes`→`ResumeThread`），因此它无法在 identity 固定前执行第一条指令。注意 `posix_spawn` 返回时 image 加载与 file actions/exec 准备**已经完成**，被门挡住的只是目标的第一条用户指令——所以这不是「exec 尚未开始」，而是「目标代码尚未执行」。slash-free program 按 **child 的有效 `PATH`**（无 `PATH` 时用 `_CS_PATH`）逐候选**真实 exec**，顺序与 `execvp` 一致（相对/空 PATH 项保持相对，由 child 在其 cwd 下解析；**不用 `access`/`metadata` 预选唯一候选**，避免 TOCTOU）：`ENOENT`/`ENOTDIR` 继续、`EACCES` 记忆并继续、`ENOEXEC` 对**该候选**走 `/bin/sh` 回退、其它错误终止；全败时按记忆的 `EACCES` → `PermissionDenied`，否则 `NotFound`。Apple 的实测细节：**PATH 搜索**中「祖先目录不可搜索」按 `ENOENT` 计（不记忆），而「可达但不可执行的文件/目录」才记忆 `EACCES`；**显式路径**不是搜索，保留内核 `EACCES`。全部形状由**差分测试**（同一 spec 分别走 frozen 与 `std::Command`，比较 exit code/stdout/typed error kind）锁住。这是「正常 spawn 还没来得及观察就已退出」的短命 child 的唯一无竞态身份来源。句柄照常占 32 槽预算且 id 单调不重用；**bound/id 耗尽在启动任何进程前拒绝**；失败路径 kill+reap，不留孤儿。`limits` 非默认时 macOS/其它 Unix **typed `Unsupported`**（`posix_spawn` 装不了 rlimit），绝不静默丢弃 |
| `process_identity(handle)` | `tool.process.identity` | status；暂存**恰好两键** `{pid,start_identity}` 的精确 identity（platform 的 opaque start token，故 PID 重用不会冒充）。`Running`/`Done` 均可读且逐字节不变；`process_spawn`（普通）的句柄**没有**冻结 identity，一律 typed 拒绝（绝不假装 frozen）；release 后/未知/stale 句柄 typed 拒绝 |
...
| 谁 | 拥有什么 |
|----|----------|
| 宿主（门） | 地址、对齐、零填充、生命周期。分配按 16 字节单元对齐（`long double` / `max_align_t` 在两套仓库 ABI 上都够），并在声明 capacity 之外保留一个不参与读回或计费的零字节；**对齐在分配本身**而不只在结构体上；输入拷到前部，调用返回即丢弃 |
...
- capacity 外的宿主零字节不是 `raw` 输出的一部分，也不把 `raw` 改成字符串契约；它只让
  分配的物理尾部不再取决于 capacity 对 16 字节对齐的余数。
...
- 只收 14 行指针签名表中**结果类型是 `i32` 的 10 行**。指针结果
...
名字被拒）——都在 `qjs_guest.rs` / `qjs_door.rs` 里。`syntax_contract.rs` 还把
`break` / `continue`、`??`、数字分隔符、四种 import、`class` / `switch` / `for…in` /
`do` / `while` 和当前 pin 的两条已知误报锁到产品编译入口与这份 README。**没锁的**：
「明确拒绝」表里除这些具名项和那组六条源码聚合断言以外的其余条目，以及门表里的编译期
参数检查 / 遮蔽 / 死分支仍发射 import 这几行，仍只是逐条编译执行记录下来的；上游一旦
放宽，它们不会自己喊出来。
