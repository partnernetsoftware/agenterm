...
/// Bounds for the menu-bar search. A menu bar sits shallow in a window's
...
///
/// UIA publishes a menu bar as an ordinary `MenuBar` element in the same
/// tree everything else comes from -- including the classic Win32 menus
/// the MSAA bridge exposes -- so this is a bounded walk and a search: no
/// menu is opened on screen and the window is never activated. What a
/// toolkit publishes for a *closed* menu is its own business; some
/// populate items only once opened, and that answer is reported as it
/// stands rather than forced.
    ...
    let Some(bar) = tree.nodes.iter().find(|node| node.role == "menu bar") else {
        return Err(AccessibilityTreeError::failed(
            ...
            if tree.truncated {
                ...
                "this window publishes no UI Automation menu bar"
            },
        ));
    };
...
/// same gate the macOS and Linux adapters apply. The press is the item's
/// own Invoke / default action, never a click at its coordinates.
pub(crate) fn invoke_menu_path(
    ...
    for (depth, segment) in path.iter().enumerate() {
        ...
        let mut matches: Vec<&AccessibilityNode> = Vec::new();
        ...
        // A menu bar item often owns a `Menu` child that holds the items,
        // so an unmatched level is retried one level down before giving up.
        if matches.is_empty() {
            for node in &menu.nodes {
                if !node.id.starts_with(&prefix) {
                    continue;
                }
                if node.id[prefix.len()..].matches('/').count() != 1 {
                    continue;
                }
                if node.name == *segment {
                    matches.push(node);
    ...
    let mark_before = menu
        .nodes
        .iter()
        .find(|node| node.id == current)
        .map(|node| toggle_word(&node.states));
    perform_node_action(window_handle, &current, AccessibilityNodeAction::Press)?;
    // UIA carries a menu item's check mark as its Toggle state, not as a
    // mark character, so the receipt speaks the words the tree already uses.
    let mark_after = menu_tree_for_window(
        window_handle,
        AccessibilityTreeBudget {
            max_depth: Some(MENU_SEARCH_DEPTH),
            max_nodes: Some(MENU_SEARCH_NODES),
...
/// `checked` / `unchecked` / `mixed` from a node's states, or `unchecked`
/// when it carries no toggle state at all.
fn toggle_word(states: &[String]) -> String {
    for word in ["mixed", "checked", "unchecked"] {
        if states.iter().any(|state| state == word) {
...
pub(crate) fn observe_window(
    ...
    Err(AccessibilityTreeError::Unsupported {
        reason: "UI Automation event subscription is not wired; the caller falls back to poll-diff"
            .into(),
    })
}
...
pub(crate) fn set_application_visibility(
    ...
    Err(AccessibilityTreeError::Unsupported {
        reason: "UI Automation has no application-level hidden state; hiding is a per-window ShowWindow operation here".into(),
    })
}
...
pub(crate) fn drag_between_nodes(
    ...
    Err(AccessibilityTreeError::Unsupported {
        reason: "named-node drag is not wired on Windows yet".into(),
    })
}
