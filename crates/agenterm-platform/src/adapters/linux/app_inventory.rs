//! linux installed-application inventory: not wired in this cut.

use crate::contract::app_inventory::{AppInventoryError, InstalledApps};

/// Listing installed applications here means walking XDG desktop-entry directories and reading a
/// display name out of each entry, and launching one means the entry's `Exec=` line.
