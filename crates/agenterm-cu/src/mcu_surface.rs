...
pub fn group_status(group_id: &str, os: &str) -> (&'static str, &'static str) {
    match group_id {
        ...
        "process" => (
            "available",
            "ps/process state/argv/cwd/environment/usage/watch/wait, one-shot fds/maps/threads/sockets on macos/linux, Linux cgroup v2 point observation, and exact-object single-process/tree signal are live where the OS has a stable provider; inspection watch/diff, policy and privileged mutation remain typed gaps",
        ),
        "resource" => (
            "available",
