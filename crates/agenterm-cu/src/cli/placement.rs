...
fn parse_expect_geometry(raw: &str) -> Result<[i32; 2], String> {
    let (width, height) = raw
        .split_once('x')
        .ok_or_else(|| format!("--expect geometry must be WIDTHxHEIGHT, got {raw:?}"))?;
    let width = width
        .parse::<i32>()
        .map_err(|_| format!("--expect width {width:?} is not an i32"))?;
    let height = height
        .parse::<i32>()
        .map_err(|_| format!("--expect height {height:?} is not an i32"))?;
    if width <= 0 || height <= 0 {
        return Err("--expect geometry width and height must be positive".into());
    }
    Ok([width, height])
}
...
fn shorthand(verb: &str, target: TargetRef, args: &mut Vec<String>) -> Result<Command, String> {
    ...
    let command = match verb {
        ...
        "unmaximize" => {
            ...
            if !args.is_empty() {
                return Err(format!(
                    "unmaximize with --expect accepts only --window H --expect unmaximized; unexpected {:?}",
                    args[0]
                ));
            }
