...
pub(super) fn simulator_status_payload(udid: &str, bundle_id: &str) -> Result<Value, CuError> {
    ...
    let status = simulator::app_status(udid, bundle_id)
        .map_err(|error| platform_error("simulator-status", error))?;
...
fn platform_error(verb: &str, error: SimulatorError) -> CuError {
    if error.kind == SimulatorErrorKind::Unsupported {
        return crate::host_limit::simulator_unsupported(verb);
    }
...
mod tests {
    ...
    fn non_macos_is_typed_unsupported_after_validation() {
        ...
        let launch = simulator_app_lifecycle_payload(
            UDID,
            "com.example.app",
            1,
            true,
        ...
        .unwrap_err();
        assert_eq!(launch.code, "simulator_unsupported");
        assert!(launch.message.contains("app lifecycle"));
    }
}
