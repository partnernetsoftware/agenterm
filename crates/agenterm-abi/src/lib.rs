...
use agenterm_platform::window_op::{
    WindowShowState, above as window_above, activate as activate_native_window, close,
    fullscreen as window_fullscreen, maximized as window_maximized, minimized as window_minimized,
    move_window, opacity as window_opacity, set_opacity, set_topmost, show, window_rect,
...
#[allow(clippy::not_unsafe_ptr_arg_deref)]
#[unsafe(no_mangle)]
pub extern "C" fn agt_a11y_node_hover(window_handle: isize, node_id: *const c_char) -> agt_status {
    fn inner(window_handle: isize, node_id: *const c_char) -> agt_status {
        if let Some(status) = a11y_mechanism_gate() {
            return status;
        }
        if node_id.is_null() {
            record_error(c"agt_a11y_node_hover", c"bad_pointer", "node_id is null");
            return agt_status::AGT_FAILED;
        }
        let node_id = match unsafe { CStr::from_ptr(node_id) }.to_str() {
            Ok(s) => s,
        ...
        match hover_node(filter, node_id) {
            Ok(()) => agt_status::AGT_OK,
            Err(e) => map_a11y_error(c"agt_a11y_node_hover", e),
        }
    }
    match catch_unwind(AssertUnwindSafe(|| inner(window_handle, node_id))) {
        Ok(s) => s,
...
#[allow(clippy::not_unsafe_ptr_arg_deref)]
#[unsafe(no_mangle)]
pub extern "C" fn agt_native_window_maximized(
    handle: isize,
    out_maximized: *mut i32,
) -> agt_status {
    fn inner(handle: isize, out_maximized: *mut i32) -> agt_status {
        if native_handle_error(c"agt_native_window_maximized", handle) {
            return agt_status::AGT_FAILED;
        }
        ...
        match window_maximized(handle) {
            Ok(value) => {
                unsafe { *out_maximized = i32::from(value) };
                agt_status::AGT_OK
            }
            Err(agenterm_platform::window_op::WindowOpError::Unsupported { .. }) => {
                agt_status::AGT_UNSUPPORTED
...
#[allow(clippy::not_unsafe_ptr_arg_deref)]
#[unsafe(no_mangle)]
pub extern "C" fn agt_native_window_above(handle: isize, out_above: *mut i32) -> agt_status {
    fn inner(handle: isize, out_above: *mut i32) -> agt_status {
        if native_handle_error(c"agt_native_window_above", handle) {
            return agt_status::AGT_FAILED;
        }
        if out_above.is_null() {
            record_error(
                c"agt_native_window_above",
                c"bad_pointer",
                "out_above is null",
            );
            return agt_status::AGT_FAILED;
        ...
        match window_above(handle) {
            Ok(value) => {
                unsafe { *out_above = i32::from(value) };
                agt_status::AGT_OK
            }
            Err(agenterm_platform::window_op::WindowOpError::Unsupported { .. }) => {
                agt_status::AGT_UNSUPPORTED
    ...
    match catch_unwind(AssertUnwindSafe(|| inner(handle, out_above))) {
        Ok(s) => s,
        Err(_) => {
            record_error(
                c"agt_native_window_above",
                c"panic",
                "panic in agt_native_window_above",
            );
            agt_status::AGT_FAILED
        }
    }
...
/// ABI 1.33: read EWMH `_NET_WM_WINDOW_OPACITY` as permille (0 =
/// transparent, 1000 = opaque).
#[allow(clippy::not_unsafe_ptr_arg_deref)]
#[unsafe(no_mangle)]
pub extern "C" fn agt_native_window_opacity(handle: isize, out_permille: *mut u32) -> agt_status {
    fn inner(handle: isize, out_permille: *mut u32) -> agt_status {
        if native_handle_error(c"agt_native_window_opacity", handle) {
            return agt_status::AGT_FAILED;
        }
        if out_permille.is_null() {
            record_error(
                c"agt_native_window_opacity",
                c"bad_pointer",
                "out_permille is null",
            );
            return agt_status::AGT_FAILED;
        }
        if !window_op_available() {
        ...
        match window_opacity(handle) {
            Ok(value) => {
                unsafe { *out_permille = value };
                agt_status::AGT_OK
            }
            Err(agenterm_platform::window_op::WindowOpError::Unsupported { .. }) => {
                agt_status::AGT_UNSUPPORTED
    ...
    match catch_unwind(AssertUnwindSafe(|| inner(handle, out_permille))) {
        Ok(s) => s,
        Err(_) => {
            record_error(
                c"agt_native_window_opacity",
                c"panic",
                "panic in agt_native_window_opacity",
            );
            agt_status::AGT_FAILED
        }
    }
...
/// ABI 1.33: write EWMH `_NET_WM_WINDOW_OPACITY`. `opacity_permille` must be
/// 0..=1000.
#[unsafe(no_mangle)]
pub extern "C" fn agt_native_window_set_opacity(
    handle: isize,
    opacity_permille: u32,
) -> agt_status {
    fn inner(handle: isize, opacity_permille: u32) -> agt_status {
        if native_handle_error(c"agt_native_window_set_opacity", handle) {
            return agt_status::AGT_FAILED;
        }
        if opacity_permille > 1000 {
            record_error(
                c"agt_native_window_set_opacity",
                c"bad_opacity",
                "opacity_permille must be 0..=1000",
            );
            return agt_status::AGT_FAILED;
        }
        if !window_op_available() {
        ...
        match set_opacity(handle, opacity_permille) {
            Ok(()) => agt_status::AGT_OK,
            Err(agenterm_platform::window_op::WindowOpError::Unsupported { .. }) => {
                agt_status::AGT_UNSUPPORTED
            }
    ...
    match catch_unwind(AssertUnwindSafe(|| inner(handle, opacity_permille))) {
        Ok(s) => s,
        Err(_) => {
            record_error(
                c"agt_native_window_set_opacity",
                c"panic",
                "panic in agt_native_window_set_opacity",
            );
            agt_status::AGT_FAILED
        }
    }
